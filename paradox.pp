...
uses
  sysutils, classes, db, pxlib, bufdataset_parser, LConvEncoding;
  ...
  if result and (buffer <> nil) then
    ...
    Case pft of
      pxfAlpha:
        ...
        If result then
          begin
          //atext := CP1251ToUTF8( Value );
          //Flen := Length( atext );
          //Move(PChar(atext)^,Buffer^, fLen );
          //Field.Size := fLen;
      ...
      pxfShort:
        begin
        Result:=PX_get_data_short(FDoc,pansichar(Buf), flen, @DS)>0;
        If result then
          PSmallInt(Buffer)^:=DS;
        end;
      pxfAutoInc,
